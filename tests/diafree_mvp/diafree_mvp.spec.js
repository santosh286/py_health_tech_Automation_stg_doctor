...
test('DiafFree MVP — Homepage to PDP (Steps 1–9)', async ({ page }) => {
  ...
  for (let i = 0; i < productCount; i++) {
    ...
    if (title.includes('Dia Free Juice - Blood Sugar Management')) {
      ...
      const chronic = await page.context().cookies();
      const setCookie = chronic.find(c => c.name === 'chronic_bucket');
      ...
      // ============================================================
      // STEP 11 — Select pack card bg-[#EDEFE5] and verify "1 Month Dia Free Juice"
      // ============================================================
      const packCard = page.locator('.bg-\\[\\#EDEFE5\\].p-\\[18px\\]').first();
      await expect(packCard, '❌ Pack card bg-[#EDEFE5] p-[18px] not visible').toBeVisible({ timeout: 10000 });
      await packCard.scrollIntoViewIfNeeded();
      await packCard.click();
      console.log('[STEP 11] ✅ Pack card clicked');

      const packTitle = packCard.getByText('1 Month Dia Free Juice');
      await expect(packTitle, '❌ "1 Month Dia Free Juice" not found in selected pack card').toBeVisible({ timeout: 5000 });
      const packText = await packCard.innerText();
      expect(packText, '❌ Pack card text does not contain "1 Month Dia Free Juice"').toContain('1 Month Dia Free Juice');
      console.log(`[STEP 11] ✅ Verified pack: "${packText.split('\n')[0].trim()}"`);
      ...
      if (shiprocketVisible) {
        ...
        console.log(`[STEP 14] ✅ Phone entered in Shiprocket: ${phone}`);
      ...
      if (isRadioVisible) {
        ...
        console.log('[STEP 19] ✅ Payment section #80000141 clicked (no radio found)');
      }

      // ============================================================
      // STEP 20 — Click Proceed to Pay
      // ============================================================
      ...
      // If "Something went wrong" — wait 1s, click Try Again once, then fail if still broken
      const somethingWrong25 = bookingCard.getByText('Something went wrong', { exact: false });
      const isWrong25 = await somethingWrong25.isVisible({ timeout: 3000 }).catch(() => false);
      if (isWrong25) {
        console.log('[STEP 25] ⚠️ "Something went wrong" detected — waiting 1s then clicking Try Again');
        await page.waitForTimeout(1000);
        const tryAgainBtn25 = bookingCard.getByRole('button', { name: /try again/i });
        await expect(tryAgainBtn25, '❌ Try Again button not found').toBeVisible({ timeout: 5000 });
        await tryAgainBtn25.click();
        await page.waitForTimeout(2000);
        console.log('[STEP 25] ✅ Try Again clicked');

        const stillWrong = await somethingWrong25.isVisible({ timeout: 3000 }).catch(() => false);
        expect(stillWrong, '❌ "Something went wrong" persists after Try Again — booking failed').toBe(false);
      }
  ...
  console.log('🎉 Steps 1–26 complete — DiafFree MVP flow verified');
});
